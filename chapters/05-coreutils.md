...
Все строки будут отсортированы по первому символу (по IP-адресу, потому что
он стоит в начале каждой строки). Строки с `10.0.0.5` окажутся вверху,
строки с `192.168.1.*` — ниже.
...
> исходный файл. Чтобы изменить файл на месте, используйте флаг `-i`:
>
> ```bash
